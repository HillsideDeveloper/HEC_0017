...
class ClinicalConsole:
    ...
    def master_control_loop(self):
        """Unified loop for Pressure PID, Temperature PID, and Watchdog Pulse."""
        while True:
            # 1. Pressure -> RPM PID
            ...
            # 2. Temperature -> Heater PID
            # SAFETY: Only run heater if pump is moving to avoid localized boiling
            if self.temp_auto_mode.get() and self.actual_rpm > 150:
                try:
                    t_out = self.temp_pid.update(float(self.temp_val))
                    self.heater_pwm.set(int(t_out))
                except: pass
            elif self.actual_rpm <= 150:
                # Interlock: Force heater off if pump stops
                if self.temp_auto_mode.get():
                    self.heater_pwm.set(0)

            # 3. Watchdog Pulse (Mandatory update to Board 1 every 2s)
    ...
    def parse_board_one(self, l):
        if "A," in l:
            ...
            try:
                ...
                p_v, f_v = float(parts[4]), float(parts[6])
                self.press_val, self.flow_val = f"{p_v:.2f}", f"{f_v:.2f}"
    ...
    def parse_terumo(self, l):
        ...
        if len(f) >= 5 and re.search(r'\d{2}:\d{2}:\d{2}', f[0]):
            ...
            self.last_terumo_packet_time = datetime.now()
            # Temp from Terumo feeds the Thermal PID
    ...
    def create_layout(self):
        ...
        # Sidebar: Blood Pump & Pressure PID
        ...
        tk.Label(b1, text="Man. Heater PWM:").pack(); tk.Scale(b1, from_=0, to=180, orient=tk.HORIZONTAL, variable=self.heater_pwm).pack(fill=tk.X)
    ...
    # (Supporting methods: terumo_listener, board_one_listener, make_led, create_inf_row, toggle_logging, etc. identical to v2.9.3)
    ...
    def start_health_monitor_thread(self):
        while True:
            threading.Event().wait(300.0); self.root.after(0, self.update_flow_graph)
