...
class PID:
    ...
    def update(self, measurement, dt=1.0):
        ...
        # Anti-windup clamp using our custom windup_limit (1500 for temperature)
...
class ClinicalConsole:
    def __init__(self, root):
        ...
        self.port_status = {"Pump": True, "Terumo": True, "Board2": True, "Board1": True}
        ...
        self.air_pump_pct = tk.IntVar(value=0)
        self.gas_valve_pct = tk.IntVar(value=0)
        ...
        # Launch Threads (CRITICAL RE-ORDERING FOR GUI THREAD SAFETY)
        ...
        # Main GUI Loop Callbacks (Must run synchronously)
        self.master_control_loop()
        self.check_heartbeat_status()

    # --- MASTER CONTROL LOOP (1Hz - Integrated Safe Architecture) ---
    def master_control_loop(self):
        ...
        if b1_data_age > 30.0:
            ...
            if not self.b1_critical_printed:
                ...
                self.log_msg(f"Board One data stream missing for {int(b1_data_age)}s.")
                self.log_msg(f"System entering permanent hardware stall watch.")
        ...
        # --- STATE 2: INTERMITTENT HANG / DATA TIME OUT (2.0 TO 30.0 SECONDS) ---
            ...
            if not self.b1_error_printed:
                self.log_msg(f"Error detected on board one.")
                self.log_msg(f"Waiting for system reset...")
                self.log_msg(f"Heater off and pump set to 1000 RPM.")
                self.log_msg(f"Standby...")
                self.b1_error_printed = True

        # --- STATE 1: NORMAL CONTROL (DATA FRESHNESS IS SOUND, < 2.0 SECONDS) ---
        else:
            # 1. Pressure PID
            b1_age = (datetime.now() - self.last_b1_receive_time).total_seconds()
        ...
        # Structural Tkinter thread timer callback (Avoids background loop freeze)
        self.root.after(1000, self.master_control_loop)
    ...
    def send_b2_gas_cmd(self):
        # Swap Air Pump and Gas Valve variable mapping to match hardware wiring. DV Changed 14/04/26
    ...
    def send_pump_cmd(self, rpm):
        if self.recovery_in_progress: return 
    ...
    def create_layout(self):
        self.top_frame = tk.Frame(self.root, height=150)
        self.top_frame.pack(side=tk.TOP, fill=tk.X)
        
        led_c = tk.Frame(self.top_frame)
        led_c.pack(side=tk.LEFT, padx=20)
        ...
        self.db_frame = tk.Frame(mid, padx=10)
        self.db_frame.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
        self.sidebar = tk.Frame(mid, width=320, bg="#f4f4f4", padx=10)
        self.sidebar.pack(side=tk.RIGHT, fill=tk.Y)
        
        db = tk.LabelFrame(self.db_frame, text=" Perfusion Dashboard ")
        ...
        for i, (k, l, c) in enumerate(ly):
            ...
            lbl = tk.Label(f, text="--.--", font=('Arial', 32, 'bold'), fg=c)
            lbl.pack(anchor="w")
            self.metrics[k] = lbl

        gf = tk.LabelFrame(self.db_frame, text=" Flow Trend ")
        gf.pack(fill=tk.BOTH, expand=True, pady=10)
        self.fig, self.ax = plt.subplots(figsize=(8, 3), dpi=90)
        self.fig.patch.set_facecolor('#f4f4f4')
        self.canvas = FigureCanvasTkAgg(self.fig, master=gf)
        self.canvas.get_tk_widget().pack(fill=tk.BOTH, expand=True)

        bp = tk.LabelFrame(self.sidebar, text=" Blood Pump (mmHg) ")
        ...
        self.press_chk.pack()
        tk.Label(bp, text="Target mmHg:").pack(side=tk.LEFT)
        tk.Entry(bp, textvariable=self.press_setpoint, width=5).pack(side=tk.LEFT)
        ...
        self.rpm_ent = tk.Entry(bp, justify='center')
        self.rpm_ent.insert(0, "1000")
        self.rpm_ent.pack()
        self.rpm_actual_lbl = tk.Label(bp, text="Actual: 0 RPM", font=('Arial', 10, 'bold'))
        self.rpm_actual_lbl.pack()

        b1 = tk.LabelFrame(self.sidebar, text=" Temperature (Celsius) ")
        ...
        self.temp_scale = tk.Scale(b1, from_=18, to=40, resolution=0.1, orient=tk.HORIZONTAL, variable=self.temp_setpoint)
        ...
        bg = tk.LabelFrame(self.sidebar, text=" Gas Control ")
        bg.pack(fill=tk.X, pady=5)
        ...
        inf = tk.LabelFrame(self.sidebar, text=" Syringe Infusion ")
        inf.pack(fill=tk.X, pady=5)
        self.create_inf_row(inf, "Upper", PORT_UPPER_SYRINGE)
        self.create_inf_row(inf, "Lower", PORT_LOWER_SYRINGE)
        
        tk.Button(self.sidebar, text="STOP ALL", bg="red", fg="white", command=self.global_emergency_stop).pack(fill=tk.X, pady=10)
        self.btn_log = tk.Button(self.sidebar, text="Start Recording", command=self.toggle_logging)
        self.btn_log.pack(fill=tk.X)
    ...
    def parse_terumo(self, l):
        ...
        if len(f) >= 5 and re.search(r'\d{2}:\d{2}:\d{2}', f[0]):
            self.health_counts["Terumo"] += 1
            self.terumo_active = True
            self.last_terumo_packet_time = datetime.now()
    ...
    def make_led(self, parent, text, color):
        ...
        c = tk.Canvas(f, width=25, height=25)
        c.pack()
    ...
    def start_syringe_watchdog_thread(self):
        while True:
            ...
            for port, state in self.syringe_states.items():
                if state == "RUN": self.check_recov(port)

    def check_recov(self, port):
        try:
            reply = self.safe_comm(port, b"\r", 0)
            if reply and b"S" in reply:  
    ...
    def board_one_listener(self):
        while True:
            try:
                with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                    # CRITICAL FIXED: Tightened read constraint to catch zombie network hangs in 2.0 seconds
                    s.settimeout(2.0)
                    s.connect((ES_IP, PORT_BOARD_1))
                    buf = ""
                    while True:
                        ...
                        while "\r" in buf: 
                            ...
                            self.parse_board_one(line.strip())
    ...
    def toggle_logging(self):
        if not self.is_logging:
            ...
            self.is_logging = False
            self.btn_log.config(text="Start Recording", bg="#f0f0f0", fg="black")
